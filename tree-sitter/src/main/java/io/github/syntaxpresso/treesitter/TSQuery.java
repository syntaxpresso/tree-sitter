...
public class TSQuery {
   ...
   * The predicates are represented as a single array of steps. There are three types of steps in
   * this array, which correspond to the three legal values for the <code>type</code> field:
   *
   * <ul>
   *   <li>`TSQueryPredicateStepTypeCapture` - Steps with this type represent names of captures.
   *       Their `value_id` can be used with the `ts_query_capture_name_for_id` function to obtain
   *       the name of the capture.
   *   <li>`TSQueryPredicateStepTypeString` - Steps with this type represent literal strings. Their
   *       `value_id` can be used with the `ts_query_string_value_for_id` function to obtain their
   *       string value.
   *   <li>`TSQueryPredicateStepTypeDone` - Steps with this type are *sentinels* that represent the
   ...
   * A non-local pattern has multiple root nodes and can match within a repeating sequence of nodes,
   * as specified by the grammar. Non-local patterns disable certain optimizations that would
   * otherwise be possible when executing a query on a specific range of a syntax tree.
   *
   * @param patternIndex The index of the pattern.
  ...
  /**
   * Check if a given pattern is guaranteed to match once a given step is reached. The step is
   * specified by its byte offset in the query's source code.
   *
   ...
   * Each capture and string is associated with a numeric id based on the order that it appeared in
   * the query's source.
   *
  ...
  /**
   * Get the quantifier of the query's captures. Each capture is * associated with a numeric id
   * based on the order that it appeared in the query's source.
   *
  ...
  public String getStringValueForId(int id) {
    ...
    for (int i = 0; i < patternCount; i++) {
      ...
      for (int j = 0; j < predicates.length; j++) {
        ...
        if (id == predicate.getValueId()
            && predicate.getType() == TSQueryPredicateStepType.TSQueryPredicateStepTypeString) {
   ...
   * This prevents the capture from being returned in matches, and also avoids any resource usage
   * associated with recording the capture. Currently, there is no way to undo this.
   *
   ...
   * This prevents the pattern from matching and removes most of the overhead associated with the
   * pattern. Currently, there is no way to undo this.
   *
