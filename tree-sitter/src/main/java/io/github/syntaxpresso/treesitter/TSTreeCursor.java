...
public class TSTreeCursor {
   ...
   * A tree cursor allows you to walk a syntax tree more efficiently than is possible using the
   * {@link TSNode} functions. It is a mutable object that is always on a certain syntax node, and
   * can be moved imperatively to different nodes. <br>
   * Note that the given node is considered the root of the cursor, and the cursor cannot walk
   * outside this node.
   *
   ...
   * This returns <code>null</code> if the current node doesn't have a field. See also {@link
   * TSNode#getChildByFieldName(String) TSNode#getChildByFieldName}.
   *
   ...
   * This returns zero if the current node doesn't have a field. See also {@link
   * TSNode#getChildByFieldId(int) TSNode#getChildByFieldId}, {@link
   * TSLanguage#fieldIdForName(String) TSLanguage#fieldIdForName}.
   *
   ...
   * Note that the node the cursor was constructed with is considered the root of the cursor, and
   * the cursor cannot walk outside this node.
   *
   ...
   *     there was no next sibling node.<br>
   */
   ...
   * Note, that this function may be slower than {@link #gotoNextSibling()} due to how node
   * positions are stored. In the worst case, this will need to iterate through all the children up
   * to the previous sibling node to recalculate its position. Also note that the node the cursor
   * was constructed with is considered the root of the cursor, and the cursor cannot walk outside
   * this node.
   *
   * @return <code>true</code> if the cursor successfully moved, and returns <code>false</code> if
  ...
  /**
   * Move the cursor to the first child of its current node that contains or starts after the given
   * byte offset.<br>
   *
   * @param startByte The byte offset.
   * @return The index of the child node if one was found, and returns -1 if no such child was
   *     found.
   */
