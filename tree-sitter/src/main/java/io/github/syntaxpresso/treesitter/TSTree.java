...
public class TSTree {
   ...
   * You need to copy a syntax tree in order to use it on more than one thread at a time, as syntax
   * trees are not thread safe.
   *
  ...
  /**
   * Edit the syntax tree to keep it in sync with source code that has been edited.<br>
   * You must describe the edit both in terms of byte offsets and in terms of (row, column)
   * coordinates.
   *
   ...
   * returning an array of ranges whose syntactic structure has changed.<br>
   ...
   * of the {@link TSParser#parse(byte[], TSTree, TSReader, TSInputEncoding) parse()} functions. You
   * need to pass the old tree that was passed to parse, as well as the new tree that was returned
   * from that function.<br>
   * The returned ranges indicate areas where the hierarchical structure of syntax nodes (from root
   * to leaf) has changed between the old and new trees. Characters outside these ranges have
   * identical ancestor nodes in both trees.<br>
   * Note that the returned ranges may be slightly larger than the exact changed areas, but
