...
import static io.github.syntaxpresso.treesitter.TSParser.*;
import static io.github.syntaxpresso.treesitter.TSParser.ts_query_cursor_next_match;
...
public class TSQueryCursor {
   ...
   * The cursor stores the state that is needed to iteratively search for matches. To use the query
   * cursor, first call {@link #exec(TSQuery, TSNode) exec()} to start running a given query on a
   * given syntax node. Then, there are two options for consuming the results of the query:
   *
   * <ol>
   *   <li>Repeatedly call {@link #nextMatch(TSQueryMatch) nextMatch()} to iterate over all of the
   *       *matches* in the order that they were found. Each match contains the index of the pattern
   *       that matched, and an array of captures. Because multiple patterns can match the same set
   *       of nodes, one match may contain captures that appear *before* some of the captures from a
   *       previous match.
   *   <li>Repeatedly call {@link #nextCapture(TSQueryMatch) nextCapture()} to iterate over all of
   *       the individual *captures* in the order that they appear. This is useful if don't care
   *       about which pattern matched, and just want a single ordered sequence of captures.
   * </ol>
   *
   * If you don't care about consuming all the results, you can stop calling {@link
   * #nextMatch(TSQueryMatch) nextMatch()} or {@link #nextCapture(TSQueryMatch) nextCapture()} at
   * any point. You can then start executing another query on another node by calling {@link
   * #exec(TSQuery, TSNode) exec()} again.
   */
  ...
  /**
   * Set the maximum number of in-progress matches allowed by this query cursor.<br>
   * Query cursors have an optional maximum capacity for storing lists of in-progress captures. If
   * this capacity is exceeded, then the earliest-starting match will silently be dropped to make
   * room for further matches. This maximum capacity is optional by default, query cursors allow any
   * number of pending matches, dynamically allocating new space for them as needed as the query is
   * executed.
   *
  ...
  /**
   * Set the maximum duration in microseconds that query execution should be allowed to take before
   * halting.<br>
   ...
   * @deprecated use {@link #execWithOptions(TSQuery, TSNode, TSQueryProgress)} and pass in a
   *     callback instead, this will be removed in 0.26.
   */
  @Deprecated
   ...
   * The query cursor will return matches that intersect with the given point range. This means that
   * a match may be returned even if some of its captures fall outside the specified range, as long
   * as at least part of the match overlaps with the range.<br>
   * For example, if a query pattern matches a node that spans a larger area than the specified
   * range, but part of that node intersects with the range, the entire match will be returned.<br>
   *
   * @param startByte The index of the start byte in the range.
   * @param endByte The index of the end byte in the range.
   * @return <code>false</code> if the start byte is greater than the end byte, otherwise it will
   ...
   * For example, if a query pattern matches a node that spans a larger area than the specified
   * range, but part of that node intersects with the range, the entire match will be returned.<br>
   * This will return `false` if the start point is greater than the end point, otherwise it will
   * return `true`.
   *
   * @param startPoint The start point of the range.
   * @param endPoint The end point of the range.
   * @return <code>false</code> if the start point is greater than the end point, otherwise it will
   *     return <code>true</code>.
   */
   ...
   * If there is a match, write it to <code>match</code> and return <code>true</code>. Otherwise,
   ...
   * If there is a capture, write its match to <code>match</code> and its index within the match's
   * capture list to <code>captureIndex</code>. Otherwise, return <code>false</code>. <br>
