...
public class TSNode {
  ...
  /**
   * Check if the node is null. Functions like {@link #getChild(int) getChild()} and {@link
   * #getNextSibling()} will return a null node to indicate that no such node was found.
   *
   ...
   * Returns <code>null</code>, if no field is found.
   *
   * @param index The index of the child to get.
  ...
  /**
   * Get the field name for node's named child at the given index, where zero represents the first
   * named child. Returns null, if no field is found.
   *
   ...
   * You can convert a field name to an id using the {@link TSLanguage#fieldIdForName(String)
   * fieldIdForName()} function.
   *
  ...
  /**
   * Get the smallest named node within this node that spans the given range of (row, column)
   * positions.
   *
   * @param startPoint the start point to search.
   * @param endPoint the end point to search.
   ...
   * This function is only rarely needed. When you edit a syntax tree with the {@link
   * TSTree#edit(TSInputEdit) TStree#edit()} function, all the nodes that you retrieve from the tree
   * afterward will already reflect the edit. You only need to use this function when you have a
   ...
   * should be used in {@link TSLanguage#nextState(int, int)} instead of {@link TSNode#getSymbol()}
   *
