...
public class TSParser {
  ...
  private static native long ts_parser_parse_string(
      long ts_parser_ptr, long ts_tree_ptr, String input);
  ...
  private static native void ts_parser_print_dot_graphs(
      long ts_parser_ptr, FileDescriptor fileDescriptor);
  ...
  protected static native TSNode ts_tree_root_node_with_offset(
  ...
  protected static native TSRange[] ts_tree_get_changed_ranges(
      long old_tree_ptr, long new_tree_ptr);
  ...
  protected static native TSNode ts_node_descendant_for_byte_range(
  ...
  protected static native TSNode ts_node_named_descendant_for_byte_range(
      TSNode node, int start_byte, int end_byte);
  ...
  protected static native int ts_tree_cursor_goto_first_child_for_byte(
      long cursor_ptr, int startByte);

  protected static native int ts_tree_cursor_goto_first_child_for_point(
      long cursor_ptr, TSPoint startPoint);
  ...
  protected static native boolean ts_query_is_pattern_non_local(
      long ts_query_ptr, int pattern_index);

  protected static native boolean ts_query_is_pattern_guaranteed_at_step(
      long ts_query_ptr, int byte_offset);

  protected static native String ts_query_capture_name_for_id(long ts_query_ptr, int index);

  protected static native int ts_query_capture_quantifier_for_id(
      long ts_query_ptr, int pattern_id, int index);
  ...
  protected static native void ts_query_cursor_exec(
      long ts_query_cursor_ptr, long ts_query_ptr, TSNode node);
  ...
  protected static native boolean ts_query_cursor_next_match(
      long ts_query_cursor_ptr, TSQueryMatch match);

  protected static native void ts_query_cursor_remove_match(long ts_query_cursor_ptr, int match_id);

  protected static native boolean ts_query_cursor_next_capture(
      long ts_query_cursor_ptr, TSQueryMatch match);

  protected static native void ts_tree_print_dot_graph(
      long ts_tree_ptr, FileDescriptor fileDescriptor);
  ...
  protected static native int ts_language_next_state(
      long ts_language_ptr, int ts_state_id, int ts_symbol);

  protected static native int ts_language_field_count(long ts_language_ptr);

  protected static native String ts_language_field_name_for_id(
      long ts_language_ptr, int ts_field_id);

  protected static native int ts_language_field_id_for_name(
      long ts_language_ptr, String field_name);
  ...
  protected static native int ts_language_symbol_for_name(
      long ts_language_ptr, String name, boolean is_named);
  ...
  protected static native boolean ts_lookahead_iterator_reset_state(
      long ts_lookahead_iterator_ptr, int ts_state_id);
  ...
  protected static native String ts_lookahead_iterator_current_symbol_name(
      long ts_lookahead_iterator_ptr);

  protected static native TSNode ts_node_child_with_descendant(TSNode node, TSNode descendant);

  protected static native String ts_node_field_name_for_named_child(
      TSNode node, long namedChildIndex);

  protected static native void ts_query_cursor_set_timeout_micros(
      long ts_query_ptr, long timeout_micros);
  ...
  /**
   * * Set the language that the parser should use for parsing. Returns a boolean indicating whether
   * the language was successfully assigned. True means assignment succeeded. False means there was
   * a version mismatch: the language was generated with an incompatible version of the Tree-sitter
   * CLI. Check the language's version using `ts_language_version` and compare it to this library's
   * {@link TSParser#TREE_SITTER_LANGUAGE_VERSION} and {@link
   * TSParser#TREE_SITTER_MIN_COMPATIBLE_LANGUAGE_VERSION} constants.
   *
  ...
  /**
   * Use the parser to parse some source code stored in one contiguous buffer. The first two
   * parameters are the same as in the {@link #parse(byte[], TSTree, TSReader, TSInputEncoding)
   * parse()} function above.
   *
   ...
   * The first four parameters work the same as in the {@link #parseString(TSTree, String)
   * parserString()} method above. The final parameter indicates whether the text is encoded as UTF8
   * or UTF16.
   *
   * @param oldTree The old tree to use. If any.
   * @param input The source code to parse.
   ...
   * If you are parsing this document for the first time, pass <code>null</code> for the `oldTree`
   * parameter. Otherwise, if you have already parsed an earlier version of this document and the
   * document has since been edited, pass the previous syntax tree so that the unchanged parts of it
   * can be reused. This will save time and memory. For this to work correctly, you must have
   * already edited the old syntax tree using the {@link TSTree#edit(TSInputEdit) edit} function in
   * a way that exactly matches the source code changes.<br>
   * This function returns a syntax tree on success, and <code>null</code> on failure. There are
   * three possible reasons for failure:<br>
   *
   * <ol>
   *   <li>The parser does not have a language assigned. Check for this using the {@link
   *       #getLanguage()} function.
   *   <li>Parsing was cancelled due to a timeout that was set by an earlier call to the {@link
   *       #setTimeoutMicros(long) setTimeoutMicros()} function. You can resume parsing from where
   *       the parser left out by calling {@link #parse(byte[], TSTree, TSReader, TSInputEncoding)
   *       parse()} again with the same arguments. Or you can start parsing from scratch by first
   *       calling {@link #reset()}.
   *   <li>Parsing was cancelled using a cancellation flag that was set by an earlier call to {@link
   *       #setCancellationFlag(long) setCancellationFlag()}. You can resume parsing from where the
   *       parser left out by calling {@link #parse(byte[], TSTree, TSReader, TSInputEncoding)
   *       parse()} again with the same arguments.
   * </ol>
   *
  ...
  public TSTree parseWithOptions(
    ...
    long oldTreePtr = oldTree == null ? 0 : oldTree.getPtr();
    long treePtr =
        ts_parser_parse_with_options(ptr, oldTreePtr, buf, reader, encoding.ordinal(), progress);
    if (treePtr == 0) {
      return null;
    }
    return new TSTree(treePtr, language);
   ...
   * By default, the parser will always include entire documents. This function allows you to parse
   * only a *portion* of a document but still return a syntax tree whose ranges match up with the
   * document as a whole. You can also pass multiple disjoint ranges.<br>
   * The second and third parameters specify the location and length of an array of ranges. The
   * parser does *not* take ownership of these ranges; it copies the data, so it doesn't matter how
   * these ranges are allocated.<br>
   * If <code>length</code> is zero, then the entire document will be parsed. Otherwise, the given
   * ranges must be ordered from earliest to latest in the document, and they must not overlap. That
   * is, the following must hold for all <code>
   *  i &lt; length - 1: ranges[i].end_byte &lt;= ranges[i + 1].start_byte </code> <br>
   * If this requirement is not satisfied, the operation will fail, the ranges will not be assigned,
   * and this function will return <code>false</code>. On success, this function returns <code>true
   * </code>
   *
   ...
   * will resume where it left off on the next call to {@link #parse(byte[], TSTree, TSReader,
   * TSInputEncoding) parse()} or other parsing functions. If you don't want to resume, and instead
   * intend to use this parser to parse some other document, you must call {@link #reset()} first.
   */
  public void reset() {
    ts_parser_reset(ptr);
  }
   ...
   * <br>
   * If parsing takes longer than this, it will halt early, returning <code>null</code>. See {@link
   * #parse(byte[], TSTree, TSReader, TSInputEncoding) parse()} for more information.
   *
   ...
   * If a non-zero value is assigned, then the parser will periodically read from this pointer
   * during parsing. If it reads a non-zero value, it will halt early, returning <code>null</code>.
  ...
  /**
   * Set the file to which the parser should write debugging graphs during parsing. The graphs are
   * formatted in the <a href ="https://graphviz.org/doc/info/lang.html">DOT</a> language. You may
   * want to pipe these graphs directly to a <code>dot(1)</code> process in order to generate SVG
   * output. You can turn off this logging by passing <code>null</code> as file.<br>
   *
   * @param file the file to which the parser should write debugging graphs. Passing <code>null
   *     </code> to disable logging and close the file if any.
